...
if ($failures !== []) {
    ...
    fwrite(STDERR, sprintf("Capability doctor self-tests failed: %d/%d\n", count($failures), $executed));
    exit(1);
}

fwrite(STDOUT, sprintf("Capability doctor self-tests passed: %d case executions\n", $executed));
exit(0);

function runCase(string $repoRoot, array $wrapper, array $case): array
{
    $tempRoot = sys_get_temp_dir() . '/testkit-capability-' . bin2hex(random_bytes(6));
    ...
    if (!mkdir($fakeBin, 0777, true) && !is_dir($fakeBin)) {
        throw new RuntimeException('Could not create fake bin dir');
    }
    if (!mkdir($testkitRoot, 0777, true) && !is_dir($testkitRoot)) {
        throw new RuntimeException('Could not create fake testkit root');
    }
    ...
    $expectedStatusLine = 'Capability doctor: ' . $case['expect_capability_status'];
    if (strpos($output, $expectedStatusLine) === false) {
        $messages[] = "Missing expected status line: {$expectedStatusLine}";
    }

    foreach (($case['require_codes'] ?? []) as $code) {
        if (strpos($output, $code) === false) {
            $messages[] = "Missing required code {$code}";
        }
    }
    foreach (($case['forbid_codes'] ?? []) as $code) {
        if (strpos($output, $code) !== false) {
            $messages[] = "Found forbidden code {$code}";
        }
    }
    foreach (($case['require_dump_fields'] ?? []) as $field) {
        if (strpos($output, $field) === false) {
            $messages[] = "Missing dump field {$field}";
        }
    }
    if (strpos($output, 'Doctor: OK') === false) {
        $messages[] = 'Doctor base did not finish in OK';
    }
