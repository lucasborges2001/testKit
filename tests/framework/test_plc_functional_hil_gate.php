...
$gate = static function (string $runtime = 'PASS', string $application = 'PASS', string $bridge = 'PASS'): array {
    return [
...
$map = ['input.raw_known' => 1200, 'input.raw_level' => 1201];
...
foreach (['FAIL', 'UNKNOWN', 'UNAVAILABLE'] as $status) {
    ...
    $assert($blocked['identities_pass'] === false, sprintf('application status %s incorrectly passed identity gate', $status));
}
...
$optOut = FunctionalHilSession::open($gate(), '127.0.0.1', $map, false, 9, 1, 100);
...
$process = proc_open([PHP_BINARY, $fixture, '--ready=' . $ready, '--count=' . $countFile], $descriptors, $pipes);
if (!is_resource($process)) {
    ...
    if ($port <= 0) {
        ...
        foreach ([['FAIL', 'failed identity'], ['UNKNOWN', 'unknown identity']] as [$identityStatus, $label]) {
            $blockedSession = FunctionalHilSession::open($gate('PASS', $identityStatus, 'PASS'), '127.0.0.1', $map, true, $port, 1, 1000);
            $assert($blockedSession->writesAllowed() === false, $label . ' enabled writes');
            try {
                $blockedSession->writeStimulus('input.raw_known', 1);
                $assert(false, $label . ' accepted write');
            } catch (ModbusTcpFunctionalHilException $e) {
                $assert($e->stage() === 'write_disabled', $label . ' did not fail before transport');
            }
            usleep(20000);
            $blockedCount = is_file($countFile) ? (int)trim((string)file_get_contents($countFile)) : -1;
            $assert($blockedCount === 0, sprintf('%s produced FC06 writes: count=%d', $label, $blockedCount));
        }

        $allowedSession = FunctionalHilSession::open($gate(), '127.0.0.1', $map, true, $port, 1, 1000);
        $assert($allowedSession->writesAllowed() === true, 'three PASS identities plus opt-in did not enable writes');
        try {
            $allowedSession->writeStimulus('not.allowlisted', 1);
            $assert(false, 'unallowlisted logical signal accepted write');
        } catch (ModbusTcpFunctionalHilException $e) {
            $assert($e->stage() === 'stimulus_not_allowed', 'unallowlisted signal stage changed');
        }
        usleep(20000);
        $countAfterRejectedId = is_file($countFile) ? (int)trim((string)file_get_contents($countFile)) : -1;
        $assert($countAfterRejectedId === 0, sprintf('unallowlisted signal produced FC06 writes: count=%d', $countAfterRejectedId));
