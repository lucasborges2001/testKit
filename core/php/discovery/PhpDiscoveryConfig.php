...
final class PhpDiscoveryConfig
{
     ...
     * @return array{tests_dir:string,discovery_options?:array<string,mixed>,warnings:array<int,array<string,mixed>>}
     */
    public static function infraPhpFromEnv(string $repoRoot): array
    {
        $legacyRel = Env::string('TK_INFRA_PHP_DIR', 'test/infra');
        $legacyRoot = Paths::normalize($repoRoot . '/' . $legacyRel);
        $legacyTestsDir = is_dir($legacyRoot . '/tests') ? ($legacyRoot . '/tests') : $legacyRoot;

        $rootsCsv = Env::string('TK_INFRA_PHP_TEST_ROOTS', '');
        if ($rootsCsv === '') {
            return [
                'tests_dir' => $legacyTestsDir,
                'warnings' => [],
        ...
        $rootPatterns = Env::csv('TK_INFRA_PHP_TEST_ROOTS');
        $excludeRootPatterns = Env::csv('TK_INFRA_PHP_TEST_EXCLUDE_ROOTS');
        $patterns = TestPatternMatcher::normalizePatterns(
            Env::csv('TK_INFRA_PHP_TEST_PATTERNS', '*.test.php'),
            ['*.test.php']
        );
        $excludePatterns = TestPatternMatcher::normalizePatterns(
            Env::csv('TK_INFRA_PHP_TEST_EXCLUDE_PATTERNS'),
            []
        );
