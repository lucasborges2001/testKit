...
final class SqlFingerprint
{
    ...
    {
        $sample = self::stripComments($sql);
        $sample = self::replaceQuotedStrings($sample);
        $sample = preg_replace('/\b[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\b/i', '?', $sample) ?? $sample;
        $sample = preg_replace('/\b\d{4}-\d{2}-\d{2}(?:[ t]\d{2}:\d{2}:\d{2}(?:\.\d+)?)?\b/i', '?', $sample) ?? $sample;
        $sample = preg_replace('/\b0x[0-9a-f]+\b/i', '?', $sample) ?? $sample;
        $sample = preg_replace('/(?<![a-zA-Z_])[-+]?\d+(?:\.\d+)?(?:e[-+]?\d+)?(?![a-zA-Z_])/i', '?', $sample) ?? $sample;
        $sample = preg_replace('/\s+/', ' ', trim($sample)) ?? trim($sample);
